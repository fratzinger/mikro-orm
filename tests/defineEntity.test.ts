...
describe('EntitySchema.setClass', () => {
  it('should allow replacing the auto-generated class with a custom class', () => {
    const UserSchema = defineEntity({
      name: 'User',
      tableName: 'users',
      properties: p => ({
    ...
    // Get the auto-generated class - note: auto-generated classes are NOT registered
    const AutoGeneratedClass = UserSchema.class;
    expect(EntitySchema.REGISTRY.get(AutoGeneratedClass)).toBeUndefined();

    // Create a custom class extending the auto-generated one
    class CustomUser extends UserSchema.class {
    ...
    expect(EntitySchema.REGISTRY.get(CustomUser)).toBe(UserSchema);
    expect(UserSchema.class).toBe(CustomUser);
    expect(UserSchema.meta.class).toBe(CustomUser);
    expect(UserSchema.meta.prototype).toBe(CustomUser.prototype);
  });

  it('should properly handle setClass when called multiple times', () => {
    const BookSchema = defineEntity({
      name: 'Book',
      tableName: 'books',
      properties: p => ({
    ...
    class CustomBook1 extends BookSchema.class {
    ...
    expect(EntitySchema.REGISTRY.get(CustomBook1)).toBe(BookSchema);

    // Second replacement - removes CustomBook1, registers CustomBook2
    BookSchema.setClass(CustomBook2);
    expect(EntitySchema.REGISTRY.get(CustomBook2)).toBe(BookSchema);
    expect(EntitySchema.REGISTRY.get(CustomBook1)).toBeUndefined();
  });

  it('should remove old class from registry when replaced with explicitly provided class', () => {
    class Author {
    ...
    expect(EntitySchema.REGISTRY.get(Author)).toBe(AuthorSchema);

    // Create an extended class
    class ExtendedAuthor extends Author {
    ...
    // Verify old class is removed and new class is registered
    expect(EntitySchema.REGISTRY.get(ExtendedAuthor)).toBe(AuthorSchema);
    expect(EntitySchema.REGISTRY.get(Author)).toBeUndefined();
  });
});
